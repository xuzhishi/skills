...
4. **写入 Outline**：定位「会议纪要」（用户指定的文档集或项目，否则默认个人文档集），直接在它下面创建「YYYY-MM-DD 会议主题」，新的在上
...
3. 目录结构（两级：会议纪要 → 会议记录，**不再建日期目录**）：
   a. 定位「会议纪要」父文档：用列集合文档树的工具（`list_collection_documents`）找标题为「会议纪要」的文档——指定了项目时找项目的直接子文档，否则找文档集顶层的；没有 → `create_document` 创建（title = 会议纪要，collectionId = 目标 collection；项目是一篇文档时再加 parentDocumentId = 项目文档）
...
- 破坏性写操作前先向用户确认：取消/修改会议、删除/移动 Outline 文档、修改文档集成员（本技能刚创建的纪要在「会议纪要」内按时间排位不算，见第四节 d）
- 不修改已有纪要的正文；纪要的标题和位置只在按迁移方法一层化、并经用户同意时调整
